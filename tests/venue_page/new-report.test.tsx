...
jest.mock("@/app/src/lib/firebase", () => ({
  auth: { currentUser: { uid: "uid-1", email: "john@example.com", getIdToken: jest.fn().mockResolvedValue("mock-token") } },
}));
jest.mock("firebase/auth", () => ({ getAuth: jest.fn(), onAuthStateChanged: jest.fn((_a: unknown, cb: (u: unknown) => void) => { cb({ uid: "uid-1", getIdToken: jest.fn().mockResolvedValue("mock-token") }); return jest.fn(); }) }));
jest.mock("@/lib/api", () => ({ createIncident: (...args: unknown[]) => mockCreateIncident(...args) }));
...
jest.mock("@/app/venue/components/OffenderPicker", () => ({ __esModule: true, default: () => <div data-testid="offender-picker" /> }));

function renderDialog(open = true) {
  const onOpenChange = jest.fn();
  const result = render(<IncidentReportDialog open={open} onOpenChange={onOpenChange} venueId="venue-1" />);
...
describe("New Report — Incident Type", () => {
  ...
  it("lists all 11 incident types in the dropdown", () => {
    ...
    expect(options.length).toBe(12); // 11 types + 1 placeholder
  });
...
describe("New Report — Severity", () => {
  ...
  it("allows selecting a severity level", async () => {
    ...
    expect(screen.getByText("HIGH").className).toContain("text-[#E84868]");
  });
});
...
describe("New Report — Description", () => {
  ...
  it("allows typing a description", async () => {
    ...
    await user.type(textarea, "A fight broke out");
    expect(textarea).toHaveValue("A fight broke out");
  });
});

describe("New Report — Keywords", () => {
  it("renders the keyword input and Add button", () => {
    renderDialog();
    expect(screen.getByPlaceholderText("Type a keyword and press Enter")).toBeInTheDocument();
    expect(screen.getByText("Add")).toBeInTheDocument();
  });

  it("adds a keyword when Add is clicked", async () => {
    const user = userEvent.setup();
    renderDialog();
  ...
  it("adds a keyword on Enter", async () => {
    const user = userEvent.setup();
    renderDialog();
    const input = screen.getByPlaceholderText("Type a keyword and press Enter");
  ...
  it("removes a keyword when X is clicked", async () => {
    const user = userEvent.setup();
    renderDialog();
    const input = screen.getByPlaceholderText("Type a keyword and press Enter");
    await user.type(input, "fight{Enter}");
    const span = screen.getByText("fight").closest("span")!;
    await user.click(within(span).getByRole("button"));
...
describe("New Report — Submission", () => {
  ...
  it("submit button is enabled when form is complete", async () => {
    ...
    await user.click(screen.getByText("HIGH"));
    await user.type(screen.getByPlaceholderText("Describe what happened..."), "Wallet stolen");
  ...
  it("calls createIncident with correct payload including offenderIds", async () => {
    mockCreateIncident.mockResolvedValue({ id: "new-inc" });
    const user = userEvent.setup();
    renderDialog();
    await user.selectOptions(screen.getByRole("combobox"), "THEFT");
    ...
    await user.type(screen.getByPlaceholderText("Describe what happened..."), "Wallet stolen");
    const kwInput = screen.getByPlaceholderText("Type a keyword and press Enter");
