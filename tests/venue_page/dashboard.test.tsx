...
jest.mock("@/lib/api", () => ({ getMe: jest.fn().mockResolvedValue({ id: "u1", firstName: "John", lastName: "Doe", email: "john@example.com", phoneNumber: null, role: "USER" }), getVenues: jest.fn().mockResolvedValue([]), getIncidents: jest.fn().mockResolvedValue([]), joinVenue: jest.fn() }));
jest.mock("@/lib/queries", () => ({
  useIncidentsQuery: jest.fn().mockReturnValue({ data: [], isLoading: false }),
...
describe("StatCard Component", () => {
  it("renders title, value, meta, and subtitle", () => {
    render(<StatCard title="ACTIVE INCIDENTS" value={5} meta="Last report 2 mins ago" subtitle="↑ 2 from Last Hour" accent="red" />);
    expect(screen.getByText("ACTIVE INCIDENTS")).toBeInTheDocument();
  ...
  it("renders without optional meta", () => {
    render(<StatCard title="NETWORK ALERTS" value={13} accent="blue" />);
    expect(screen.getByText("NETWORK ALERTS")).toBeInTheDocument();
    expect(screen.getByText("13")).toBeInTheDocument();
  });

  it.each(["red", "amber", "green", "blue"] as const)("renders with %s accent", (accent) => {
    render(<StatCard title="TEST" value={0} accent={accent} />);
    expect(screen.getByText("TEST")).toBeInTheDocument();
  });

  it("renders a progress bar when progress prop is provided", () => {
    const { container } = render(<StatCard title="CAP" value={200} accent="green" progress={0.75} />);
