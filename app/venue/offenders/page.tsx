...
import OffenderDetailModal from "../components/OffenderDetailModal";
  ...
  const columns = useMemo<ColumnDef<OffenderResponse>[]>(() => [
    ...
    },
    {
      ...
      header: ({ column }) => (
        <span className="flex items-center">
          Risk Score <SortButton column={column} />
        </span>
      ),
      cell: ({ getValue }) => {
        ...
        const color =
          score >= 8 ? "text-[#E84868]" : score >= 5 ? "text-[#DBA940]" : "text-[#5B6AFF]";
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return v ? (
          <span className="rounded-md border border-[#2A2A34] bg-[#1a1a28] px-2 py-0.5 text-[11px] text-[#DDDBDB]">
            {v}
          </span>
        ) : (
    ...
    },
    {
      ...
      header: "Notes",
      cell: ({ getValue }) => (
        <span className="max-w-[220px] truncate block text-[#8B8B9D] text-xs">
          {(getValue() as string) || "—"}
        </span>
      ),
    },
    ...
    },
    {
      ...
      header: ({ column }) => (
        <span className="flex items-center whitespace-nowrap">
          Added <SortButton column={column} />
        </span>
      ),
      cell: ({ getValue }) => (
        <span className="text-[#8B8B9D] whitespace-nowrap text-xs">{formatDate(getValue() as string)}</span>
      ),
    },
    {
      ...
      header: ({ column }) => (
        <span className="flex items-center whitespace-nowrap">
          Last Updated <SortButton column={column} />
        </span>
      ),
      cell: ({ getValue }) => (
        <span className="text-[#8B8B9D] whitespace-nowrap text-xs">{formatDate(getValue() as string)}</span>
            ...
            {table.getHeaderGroups().map((hg) => (
              <TableRow key={hg.id} className="border-[#2A2A34] hover:bg-transparent">
                {hg.headers.map((header) => (
                  <TableHead
                    key={header.id}
                    className="text-[10px] font-bold uppercase text-[#8B8B9D]"
                  >
